...
def _run_single_simulation(sim_id, conf_int, lower_z_thresh, upper_z_thresh, current_rnd, player_df_raw, price_df, price_model_obj_1, price_model_obj_2, price_model_obj_3, optimise_fn_efp):
    """
    ...
    player_df = player_df_init.groupby(['Name', 'Price', "Team", "Round", "Wk_f", "Bat_f", "Bowl_f", "Role", "weight","Available", "In_Team"], as_index=False).agg(
    ...
    pts_per_game = player_df_init[['Name', 'game_num', 'exp_points']]
    ...
    # For loop for all BBL players in season
    for j in bbl15_play_list:
        ...
        # List of all rounds played by player
        ...
        for k in game_num_list:
            ...
            # Last 2 Games Moving Average Points
            if k >= 2:
            ...
            # Last 3 Games Moving Average Points
            if k >= 3:
            ...
            # Add all during season attributes to main dataframe
    ...
    player_team = player_df_init.groupby(['Name', 'Team'])['exp_points'].count().reset_index().drop(['exp_points'], axis=1)
    bbl15_game_pts_table = pd.merge(bbl15_game_pts_table_pre, player_team, on = 'Name')

    # Get unique team round game counts
    game_rnd_team_df = player_df_init.groupby(['Team','Round','game_num'])['exp_points'].count().reset_index().drop(['exp_points'], axis=1)
    ...
    # Price prediction and preparation (existing code continues)
    ...
    del bbl15_game_pts_table_pre, bbl15_game_pts_table, player_df_lags, player_df_new, curr_rnd_player_df_lags
    del curr_rnd_player_df_price_1, curr_rnd_player_df_price_2, curr_rnd_player_df_price_3, player_df_price, player_df_pred_price
