...
def roll_rnd_price_fn(player_df_init, price_df, current_rnd, 
    ...
    player_df = player_df_init_2.groupby(['Name', 'Price', "Team", "Round", "Wk_f", "Bat_f", "Bowl_f", "Role", "weight","Available", "In_Team", "On_Bench", "Trade_In", "Trade_Out", "Lock_f"], as_index=False).agg(
    ...
    curr_rnd_lookup = player_df_init_2[player_df_init_2['Round'] == 1][['Name', 'Team', 'Price','Wk_f','Bat_f','Bowl_f','Role','weight','In_Team','Lock_f', 'On_Bench', "Trade_In", "Trade_Out"]].drop_duplicates()
    ...
    player_df.loc[player_df['Round'] == current_rnd, 'On_Bench'] = player_df.loc[player_df['Round'] == current_rnd, 'On_Bench_curr'].fillna(player_df.loc[player_df['Round'] == current_rnd, 'On_Bench']) # -------------- During Rnd Specific Change ---------------
    player_df = player_df.drop(columns=['Price_curr', 'Team_curr', 'Wk_f_curr', 'Bat_f_curr', 'Bowl_f_curr', 'Role_curr', 'weight_curr', 'In_Team_curr', 'On_Bench_curr', "Lock_f_curr","Trade_In_curr", "Trade_Out_curr"])
    ...
    player_df['In_Team'] = player_df.groupby('Name')['In_Team'].ffill()
    player_df['On_Bench'] = player_df.groupby('Name')['On_Bench'].fillna(0) # -------------- During Rnd Specific Change ---------------
    player_df['Lock_f'] = player_df.groupby('Name')['Lock_f'].fillna(0) # -------------- During Rnd Specific Change ---------------
    player_df['Trade_In'] = player_df.groupby('Name')['Trade_In'].fillna(0) # -------------- During Rnd Specific Change ---------------
    player_df['Trade_Out'] = player_df.groupby('Name')['Trade_Out'].fillna(0) # -------------- During Rnd Specific Change ---------------
...
def optimise_setup_fn(
    player_df_r5, player_df_r6, player_df_r7, player_df_r8, player_df_r9, init_budget
): 
    # Round 1
    ...
    # cnt_r2, max_player_r2 = 15, range(len(price_r2)) # Hard code as strategy set to 15 players
    ...
    # team_play_cnt_r2, total_team_player_r2 = 12, range(len(price_r2)) # At least 12 players from round 1 to be in round 2 team
    ...
    # cnt_r3, max_player_r3 = 15, range(len(price_r3)) # Hard code as strategy set to 15 players
    ...
    # budget_r3, total_budget_r3 = init_budget, range(len(price_r3))
    # team_play_cnt_r3, total_team_player_r3 = 12, range(len(price_r3)) # At least 12 players from round 2 to be in round 3 team
    ...
    # cnt_r4, max_player_r4 = 15, range(len(price_r4)) # Hard code as strategy set to 15 players
    ...
    # budget_r4, total_budget_r4 = init_budget, range(len(price_r4))
    # team_play_cnt_r4, total_team_player_r4 = 12, range(len(price_r4)) # At least 12 players from round 3 to be in round 4 team
    ...
    on_bench_r5 = player_df_r5["On_Bench"] # -------------- During Rnd Specific Change ---------------
    lock_f_r5 = player_df_r5["Lock_f"] # -------------- During Rnd Specific Change ---------------
    trade_in_r5 = player_df_r5["Trade_In"] # -------------- During Rnd Specific Change ---------------
    trade_out_r5 = player_df_r5["Trade_Out"] # -------------- During Rnd Specific Change ---------------
    ...
    cnt_r5, max_player_r5 = 15, range(len(price_r5)) # Hard code as strategy set to 15 players
    ...
    budget_r5, total_budget_r5 = init_budget, range(len(price_r5))
    team_play_cnt_r5, total_team_player_r5 = 12, range(len(price_r5)) # At least 12 players from round 4 to be in round 5 team
    ...
    cnt_r6, max_player_r6 = 15, range(len(price_r6)) # Hard code as strategy set to 15 players
    ...
    budget_r6, total_budget_r6 = init_budget, range(len(price_r6))
    team_play_cnt_r6, total_team_player_r6 = 12, range(len(price_r6)) # At least 12 players from round 5 to be in round 6 team
    ...
    cnt_r7, max_player_r7 = 15, range(len(price_r7)) # Hard code as strategy set to 15 players
    ...
    budget_r7, total_budget_r7 = init_budget, range(len(price_r7))
    team_play_cnt_r7, total_team_player_r7 = 12, range(len(price_r7)) # At least 12 players from round 6 to be in round 7 team
    ...
    cnt_r8, max_player_r8 = 15, range(len(price_r8)) # Hard code as strategy set to 15 players
    ...
    budget_r8, total_budget_r8 = init_budget, range(len(price_r8))
    team_play_cnt_r8, total_team_player_r8 = 12, range(len(price_r8)) # At least 12 players from round 7 to be in round 8 team
    ...
    cnt_r9, max_player_r9 = 15, range(len(price_r9)) # Hard code as strategy set to 15 players
    ...
    budget_r9, total_budget_r9 = init_budget, range(len(price_r9))
    team_play_cnt_r9, total_team_player_r9 = 12, range(len(price_r9)) # At least 12 players from round 8 to be in round 9 team

    # Validation checks for Round 5 setup
    print("\n" + "="*80)
    print("ROUND 5 SETUP VALIDATION")
    print("="*80)
    
    # Count constraints
    trade_in_count = trade_in_r5.sum()
    trade_out_count = trade_out_r5.sum()
    in_team_locked_count = ((in_team_r5 == 1) & (lock_f_r5 == 1)).sum()
    on_bench_count = on_bench_r5.sum()
    available_count = available_r5.sum()
    
    print(f"Trade-in players (must SELECT): {trade_in_count}")
    print(f"Trade-out players (must NOT select): {trade_out_count}")
    print(f"In-team locked players (must keep): {in_team_locked_count}")
    print(f"On-bench locked players: {on_bench_count}")
    print(f"Total available players: {available_count}")
    print(f"Budget: ${budget_r5:,}")
    
    # Check for conflicts
    print("\nCONFLICT CHECKS:")
    
    # Check 1: Trade-in players that are not available
    trade_in_unavailable = ((trade_in_r5 == 1) & (available_r5 == 0)).sum()
    if trade_in_unavailable > 0:
        print(f"⚠️  WARNING: {trade_in_unavailable} trade-in players are NOT available!")
        conflicted_players = player_df_r5[(trade_in_r5 == 1) & (available_r5 == 0)][['Name', 'Team', 'Available', 'Trade_In', 'Lock_f']]
        print(conflicted_players)
    else:
        print(f"✓ All trade-in players are available")
    
    # Check 2: Trade-out players that are in-team and locked
    trade_out_locked = ((trade_out_r5 == 1) & (in_team_r5 == 1) & (lock_f_r5 == 1)).sum()
    if trade_out_locked > 0:
        print(f"⚠️  WARNING: {trade_out_locked} trade-out players are locked in team!")
        conflicted_players = player_df_r5[(trade_out_r5 == 1) & (in_team_r5 == 1) & (lock_f_r5 == 1)][['Name', 'Team', 'In_Team', 'Trade_Out', 'Lock_f']]
        print(conflicted_players)
    else:
        print(f"✓ No trade-out players are locked in team")
    
    # Check 3: Players that are both trade-in and trade-out
    both_trade = ((trade_in_r5 == 1) & (trade_out_r5 == 1)).sum()
    if both_trade > 0:
        print(f"⚠️  ERROR: {both_trade} players marked as BOTH trade-in AND trade-out!")
        conflicted_players = player_df_r5[(trade_in_r5 == 1) & (trade_out_r5 == 1)][['Name', 'Team', 'Trade_In', 'Trade_Out']]
        print(conflicted_players)
    else:
        print(f"✓ No players marked as both trade-in and trade-out")
    
    # Check 4: Minimum required selections
    min_required = in_team_locked_count + trade_in_count
    print(f"\nMINIMUM REQUIRED SELECTIONS: {min_required} players (locked + trade-in)")
    if min_required > 15:
        print(f"⚠️  ERROR: Minimum required selections ({min_required}) exceeds squad limit (15)!")
    elif min_required > cnt_r5:
        print(f"⚠️  ERROR: Minimum required selections ({min_required}) exceeds squad size ({cnt_r5})!")
    else:
        print(f"✓ Minimum selections feasible ({min_required} <= {cnt_r5})")
    
    # Check 5: Budget feasibility estimate
    if min_required > 0:
        min_required_players = player_df_r5[(in_team_r5 == 1) & (lock_f_r5 == 1) | (trade_in_r5 == 1)]
        if len(min_required_players) > 0:
            min_budget_required = min_required_players.groupby('Name')['Price'].first().sum()
            print(f"\nMINIMUM BUDGET REQUIRED: ${min_budget_required:,} (for locked + trade-in players)")
            if min_budget_required > budget_r5:
                print(f"⚠️  ERROR: Minimum budget required (${min_budget_required:,}) exceeds budget (${budget_r5:,})!")
            else:
                remaining_budget = budget_r5 - min_budget_required
                print(f"✓ Budget feasible. Remaining budget: ${remaining_budget:,}")
    ...
    return    points_r5, price_r5, weight_r5, in_team_r5, on_bench_r5, lock_f_r5, trade_in_r5, trade_out_r5, available_r5, wk_weight_r5, bat_weight_r5, bowl_weight_r5, \
...
def optimise_fn_efp(
        ...
        # budget_r4, total_budget_r4, team_play_cnt_r4, total_team_player_r4, player_df_r4, cnt_r4, max_player_r4,
        # Round 5
        points_r5, price_r5, weight_r5, in_team_r5, on_bench_r5, lock_f_r5, trade_in_r5, trade_out_r5, # -------------- During Rnd Specific Change ---------------
        available_r5, wk_weight_r5, bat_weight_r5, bowl_weight_r5,
        play_cnt_r5, total_player_r5, wk_cnt_r5, total_wk_r5, bat_cnt_r5, total_bat_r5, bowl_cnt_r5, total_bowl_r5,
        budget_r5, total_budget_r5, team_play_cnt_r5, total_team_player_r5, player_df_r5, cnt_r5, max_player_r5,
        # Round 6
        points_r6, price_r6, weight_r6, in_team_r6, available_r6, wk_weight_r6, bat_weight_r6, bowl_weight_r6,
    ...
    # x[i] = 1 if player i is selected in roster (15 total)
    ...
    # Round 1 - (Round Completed)
    # x_r1 = [m.add_var(var_type=BINARY) for i in total_player_r2]  # Selected (15)
    ...
    # p_r3 = [m.add_var(var_type=BINARY) for i in total_player_r3] # Playing (12)
    # y_r3 = [m.add_var(var_type=BINARY) for i in total_player_r3] # Captain (1)
    # Round 4 (Previous Round)
    x_r4 = [m.add_var(var_type=BINARY) for i in total_player_r5] # Selected (15)
    for i in total_player_r5:
        m += x_r4[i] == int(in_team_r5.iloc[i])
    # Round 5
    x_r5 = [m.add_var(var_type=BINARY) for i in total_player_r5] # Selected (15)
    p_r5 = [m.add_var(var_type=BINARY) for i in total_player_r5] # Playing (12)
    y_r5 = [m.add_var(var_type=BINARY) for i in total_player_r5] # Captain (1)
    # Round 6
    x_r6 = [m.add_var(var_type=BINARY) for i in total_player_r6] # Selected (15)
    p_r6 = [m.add_var(var_type=BINARY) for i in total_player_r6] # Playing (12)
    y_r6 = [m.add_var(var_type=BINARY) for i in total_player_r6] # Captain (1)
    # Round 7
    x_r7 = [m.add_var(var_type=BINARY) for i in total_player_r7] # Selected (15)
    p_r7 = [m.add_var(var_type=BINARY) for i in total_player_r7] # Playing (12)
    y_r7 = [m.add_var(var_type=BINARY) for i in total_player_r7] # Captain (1)
    # Round 8
    x_r8 = [m.add_var(var_type=BINARY) for i in total_player_r8] # Selected (15)
    p_r8 = [m.add_var(var_type=BINARY) for i in total_player_r8] # Playing (12)
    y_r8 = [m.add_var(var_type=BINARY) for i in total_player_r8] # Captain (1)
    # Round 9
    x_r9 = [m.add_var(var_type=BINARY) for i in total_player_r9] # Selected (15)
    p_r9 = [m.add_var(var_type=BINARY) for i in total_player_r9] # Playing (12)
    y_r9 = [m.add_var(var_type=BINARY) for i in total_player_r9] # Captain (1)
    ...
    rounds_data = [
        # (player_df_r2, player_df_r2, x_r1, x_r2, {}, team_play_cnt_r2, "r1_r2"), #Same DFs as current round 
        # (player_df_r3, player_df_r3, x_r2, x_r3, {}, team_play_cnt_r3, "r2_r3"), #Same DFs as current round 
        # (player_df_r4, player_df_r4, x_r3, x_r4, {}, team_play_cnt_r4, "r3_r4"), #Same DFs as current round 
        (player_df_r5, player_df_r5, x_r4, x_r5, {}, team_play_cnt_r5, "r4_r5"), #Same DFs as current round
    ...
    for curr_df, next_df, curr_x, next_x, z_dict, team_min, label in rounds_data:
        ...
        if shared_pairs_rnd:
            ...
            # Trade Boost Constraint (4 Trades Allowed for 2 Rounds (Only 11 In Team Players Required))
    ...
    m.objective = maximize(obj_r5 + obj_r6 + obj_r7 + obj_r8 + obj_r9)
    ...
    # Round 2
    # m += xsum(x_r2[i] for i in total_player_r2) == cnt_r2 # total squad players
    # m += xsum(weight_r2[i] * p_r2[i] for i in total_player_r2) == play_cnt_r2 # 12 playing players
    # m += xsum(y_r2[i] for i in total_player_r2) == 1 # Only one captain
    # Round 3
    # m += xsum(x_r3[i] for i in total_player_r3) == cnt_r3 # total squad players
    # m += xsum(weight_r3[i] * p_r3[i] for i in total_player_r3) == play_cnt_r3 # 12 playing players
    # m += xsum(y_r3[i] for i in total_player_r3) == 1 # Only one captain
    # Round 4
    # m += xsum(x_r4[i] for i in total_player_r4) == cnt_r4 # total squad players
    # m += xsum(weight_r4[i] * p_r4[i] for i in total_player_r4) == play_cnt_r4 # 12 playing players
    # m += xsum(y_r4[i] for i in total_player_r4) == 1 # Only one captain
    # Round 5
    m += xsum(x_r5[i] for i in total_player_r5) == cnt_r5
    m += xsum(weight_r5[i] * p_r5[i] for i in total_player_r5) == play_cnt_r5 # 12 playing players
    m += xsum(y_r5[i] for i in total_player_r5) == 1 # Only one captain
    # Round 6
    m += xsum(x_r6[i] for i in total_player_r6) == cnt_r6
    m += xsum(weight_r6[i] * p_r6[i] for i in total_player_r6) == play_cnt_r6 # 12 playing players
    ...
    for i in total_player_r5: # -------------- During Rnd Specific Change ---------------
        if in_team_r5[i] == 1 and lock_f_r5[i] == 1:
            m += x_r5[i] == 1  # All in-team locked players must be selected
            if on_bench_r5[i] == 0:
                m += p_r5[i] == 1  # Locked in-team players not on bench must be playing
        if on_bench_r5[i] == 1:
            m += p_r5[i] == 0  # Bench-locked players must NOT be playing
        if trade_in_r5[i] == 1:
            m += x_r5[i] == 1  # Trade-in locked players MUST be selected
        if trade_out_r5[i] == 1:
            m += x_r5[i] == 0  # Trade-out locked players must NOT be selected
            
        # Team composition constraints (apply to playing players p, not all selected x)
        # Remove bench discount as captured in budget constraint
    # Round 1  
    ...
    # m += xsum(price_r2[i] * x_r2[i] for i in total_budget_r2) - 39500*(cnt_r2 - play_cnt_r2) <= budget_r2
     # Round 3
    ...
    if m.status != OptimizationStatus.OPTIMAL and m.status != OptimizationStatus.FEASIBLE:
        ...
        # Check trade constraints in Round 5
        trade_in_players = [i for i in total_player_r5 if trade_in_r5[i] == 1]
        trade_out_players = [i for i in total_player_r5 if trade_out_r5[i] == 1]
        in_team_locked = [i for i in total_player_r5 if in_team_r5[i] == 1 and lock_f_r5[i] == 1]
        on_bench_locked = [i for i in total_player_r5 if on_bench_r5[i] == 1]
        
        print(f"Trade-in players (must be selected): {len(trade_in_players)}")
        print(f"Trade-out players (must NOT be selected): {len(trade_out_players)}")
        print(f"In-team locked players: {len(in_team_locked)}")
        print(f"On-bench locked players: {len(on_bench_locked)}")
        print(f"Available budget: {budget_r5}")
        
        raise ValueError("Optimization problem is infeasible. Check constraints, especially trade_in/trade_out flags and budget.")
    ...
    # Round 2 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    # Round 3 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    # Round 4 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    # Round 5 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    # Round 6 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    # Round 7 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    # Round 8 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    # Round 9 - Extract selected (x=1), playing (p=1), benched (x=1 but p=0)
    ...
    sel_player_df = pd.concat([sel_player_df_r5, sel_player_df_r6, sel_player_df_r7,sel_player_df_r8, sel_player_df_r9], ignore_index=True)
    ...
    return sel_player_df, sel_player_df_r5, sel_player_df_r6, sel_player_df_r7,sel_player_df_r8, sel_player_df_r9

def _run_single_sfp_sim(sim_id, conf_int, lower_z_thresh, upper_z_thresh, current_rnd, sim_budget, player_df_raw, price_df, price_model_obj_1, price_model_obj_2, price_model_obj_3, optimise_fn_efp):
    """
    ...
    # play_cnt_r4, total_player_r4, wk_cnt_r4, total_wk_r4, bat_cnt_r4, total_bat_r4, bowl_cnt_r4, total_bowl_r4, \
    # budget_r4, total_budget_r4, team_play_cnt_r4, total_team_player_r4, player_df_r4, cnt_r4, max_player_r4, \
    points_r5, price_r5, weight_r5, in_team_r5, on_bench_r5, lock_f_r5, trade_in_r5, trade_out_r5, available_r5, wk_weight_r5, bat_weight_r5, bowl_weight_r5, \
    play_cnt_r5, total_player_r5, wk_cnt_r5, total_wk_r5, bat_cnt_r5, total_bat_r5, bowl_cnt_r5, total_bowl_r5, \
    budget_r5, total_budget_r5, team_play_cnt_r5, total_team_player_r5, player_df_r5, cnt_r5, max_player_r5, \
    points_r6, price_r6, weight_r6, in_team_r6, available_r6, wk_weight_r6, bat_weight_r6, bowl_weight_r6, \
    play_cnt_r6, total_player_r6, wk_cnt_r6, total_wk_r6, bat_cnt_r6, total_bat_r6, bowl_cnt_r6, total_bowl_r6, \
    ...
    budget_r9, total_budget_r9, team_play_cnt_r9, total_team_player_r9, player_df_r9, cnt_r9, max_player_r9 =  optimise_setup_fn(
        player_df_r5, player_df_r6, player_df_r7, player_df_r8, player_df_r9, sim_budget)
    
    # b. Run optimization
    sel_player_df, sel_player_df_r5, sel_player_df_r6, sel_player_df_r7,sel_player_df_r8, sel_player_df_r9 = optimise_fn_efp(
        # Round 1
        # points_r1, price_r1, weight_r1, in_team_r1, available_r1, wk_weight_r1, bat_weight_r1, bowl_weight_r1,
        # play_cnt_r1, total_player_r1, wk_cnt_r1, total_wk_r1, bat_cnt_r1, total_bat_r1, bowl_cnt_r1, total_bowl_r1,
        # budget_r1, total_budget_r1, player_df_r1, cnt_r1, max_player_r1,
    ...
    sim_sel_players = sel_player_df_r5[['Name']]
    sim_sel_players['Simulation'] = sim_id + 1

    # Drop dataframes to free up memory
    del player_df_init, sel_player_df_r5, sel_player_df_r6, sel_player_df_r7,sel_player_df_r8, sel_player_df_r9, sel_player_df
    del price_r5, price_r6, price_r7, price_r8, price_r9
    del weight_r5, weight_r6, weight_r7, weight_r8, weight_r9
    del in_team_r5, in_team_r6, in_team_r7, in_team_r8, in_team_r9
    del available_r5, available_r6, available_r7, available_r8, available_r9
    del wk_weight_r5, wk_weight_r6, wk_weight_r7, wk_weight_r8, wk_weight_r9
    del bat_weight_r5, bat_weight_r6, bat_weight_r7, bat_weight_r8, bat_weight_r9
    del bowl_weight_r5, bowl_weight_r6, bowl_weight_r7, bowl_weight_r8, bowl_weight_r9
    del play_cnt_r5, play_cnt_r6, play_cnt_r7, play_cnt_r8, play_cnt_r9
    del budget_r5, budget_r6, budget_r7, budget_r8, budget_r9
    del total_player_r5, total_player_r6, total_player_r7, total_player_r8, total_player_r9
    del total_budget_r5, total_budget_r6, total_budget_r7, total_budget_r8, total_budget_r9
    del team_play_cnt_r5, team_play_cnt_r6, team_play_cnt_r7, team_play_cnt_r8, team_play_cnt_r9
    del total_team_player_r5, total_team_player_r6, total_team_player_r7, total_team_player_r8, total_team_player_r9
    del points_r5, points_r6, points_r7, points_r8, points_r9
    del player_df_r5, player_df_r6, player_df_r7, player_df_r8, player_df_r9
    ...
    if use_parallel:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(
                    _run_single_sfp_sim,
                    i, conf_int, lower_z_thresh, upper_z_thresh, current_rnd, sim_budget, 
